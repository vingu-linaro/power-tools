...
#
#
#Tis script monitors the entry latency of the differents C-state of the CPU of
#the system. Configuration are available is .cfg file

. ./latency_entry_monitor.cfg
if [ -r ~/.latency_entry_monitor.cfg ]; then
  . ~/.latency_entry_monitor.cfg
fi
...
while [ $PERIOD -le $PERIOD_MIN ]; do

PREV=`cat latency-run.json | grep sleep | awk '{ print $3 }' | sed s/,// | tail -n 1`
sed 's/"sleep" : '$PREV'/"sleep" : '$PERIOD'/' -i latency-run.json
...
for i in `ls ./latency-state$STATE-freq$FREQ-$PERIOD-$j/latency-run-thread*`; do cat $i | tail -n +3 | head -n -1 | awk -v max=0 -v min=5000 '{ total += $4; total2 += $4*$4;  if($4>max){max=$4}; if($4<min){min=$4} } END { print min " " total/NR " " max " " NR " " sqrt(total2*NR - total*total)/NR}'; done
